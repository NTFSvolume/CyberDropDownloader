...
def add_request_log_hooks(trace_configs: list[aiohttp.TraceConfig], request_log_hooks_name: str) -> None:
    async def on_request_start(*args):
        params: aiohttp.TraceRequestStartParams = args[2]
        log_debug(f"Starting {request_log_hooks_name} {params.method} request to {params.url}", 10)

    async def on_request_end(*args):
        params: aiohttp.TraceRequestEndParams = args[2]
        msg = f"Finishing {request_log_hooks_name}  {params.method} request to {params.url}"
